...
class MetricsDatabase:
    ...
    def _init_database(self):
        ...
        # fans_data existed -- CREATE TABLE IF NOT EXISTS above is a no-op
        # against an already-existing table, so an older DB needs the
        # columns added explicitly. Appended last so existing column
        # indices don't shift.
        cursor.execute("PRAGMA table_info(metrics)")
        existing_columns = {row[1] for row in cursor.fetchall()}
        if "network_data" not in existing_columns:
            cursor.execute("ALTER TABLE metrics ADD COLUMN network_data TEXT")
        if "fans_data" not in existing_columns:
            cursor.execute("ALTER TABLE metrics ADD COLUMN fans_data TEXT")
    ...
    def insert_metrics(self, metrics: Dict):
        ...
        def _do_insert():
            ...
            fans_data = json.dumps(metrics.get('fans', {}))
    ...
    def query_metrics(self, start_time: Optional[int] = None,
        ...
        try:
            ...
            # never stored as columns. gpu/storage/ml/network/fans/
            # bottlenecks/anomalies are stored as JSON blobs already
            # matching metrics/schema.py, so they need no remapping.
            results = []
            for row in rows:
                results.append({
                    'timestamp': row[0],
                    ...
                    'fans': json.loads(row[15]) if row[15] else {},
                })
...
if __name__ == "__main__":
    ...
    def make_metrics(ts):
        return {
            'timestamp': ts,
            'gpu': [], 'cpu': {}, 'memory': {}, 'storage': {}, 'ml': {}, 'network': {}, 'fans': {},
